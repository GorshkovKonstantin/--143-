...
int main()
{
   ...
   cout << "Введите данные о движении путника:" << end1;
...
double firstHa1fTime = calculateFirstHa1fTime(t1, v1, t2, v2, t3, v3);

cout << "Время, за которое путник одолел первую половину пути: " << firstHalfTime ‹‹ " часов." ‹‹ end1;
...
double getValue()
{
   ...
   if (cin. fail())
   {
   cout << "Некорректное значение" << end1;
   abort();
   }
   return value;
}

double ca1cu1ateFirstHa1fTimedouble t1, double v1, double t2, double v2, double t3, double v3) {
   ...
   double totalDistance = distancel + distance2 + distance; // Общее расстояние 
   double ha1fDistance = tota1Distance / 2; // Половина пути
   ...
   if (halfDistance <= distance1) {
      timeSpent = halfDistance / vl; // Если первая половина пути укладывается в первую часть
   }
   else {
      timeSpent += t1; // Учитываем все время первой части halfDistance -= distancel; 
      ha1fDistance -= distance1;// Вычитаем пройденное расстояние
      if (ha1fDistance <= distance2) {
          timeSpent += halfDistance / v2; // Вторая часть пути
      }
      else {
          timeSpent += t2; // Учитываем все время второй части
          ha1fDistance-= distance2; // Вычитаем пройденное расстояние
          timeSpent += ha1fDistance / v3; // Остаток пути на третьей части
      }
   }
   return timeSpent;
}